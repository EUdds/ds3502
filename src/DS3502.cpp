...
void DS3502::set_wiper(uint8_t wiper_value) {
    ...
    if (writeI2CWrapper(this->i2c, this->addr, DS3502_WIPER_REG, buf, 1))
    {
        printf("Failed to set wiper!\n\r");
    }
...
uint8_t DS3502::get_wiper() {
    char buf[] = {};
    if (readI2CWrapper(this->i2c, this->addr, DS3502_WIPER_REG, buf, 1))
    {
        printf("Failed to read wiper!!\n\r");
    }
